...
const getAll = catchError(async(req, res) => {
    const results = await ImgDataObservations.findAll();
...
const create = catchError(async(req, res) => {
    ...
    const image = await ImgDataObservations.create({
        imageUrl: url,
...
const getOne = catchError(async(req, res) => {
    const { id } = req.params;
    const result = await ImgDataObservations.findByPk(id);
...
const remove = catchError(async(req, res) => {
    const { id } = req.params;
    const image = await ImgDataObservations.findByPk(id);
